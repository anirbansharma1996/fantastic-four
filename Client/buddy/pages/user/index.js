...
export default function SocialProfileSimple() {
  ...
  const [modalData,setModalData]=useState({})
   useEffect(()=>{
     axios.get("http://fakestoreapi.com/products/").then((res)=>setData(res.data))
   },[])
...
<Center  width="80%" margin={"auto"} >
    ...
    {
      data.length ==0 || null ? <NoPost/>
      :

      <SimpleGrid  margin={"auto"} marginTop="40px"  columns={[1,2,3,4]} spacing={4} width={"80%"}   >
      {
       
       data.map((item)=>{
         return <>
          <Box  onClick={()=>handleAddToCart(item)} boxShadow={'2xl'} margin={"auto"}  width={"90%"} height="250px" >
